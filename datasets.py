...
def normalization(pcl):
    """
        pcl: (N, 3)
    """
    shape_scale = np.max([np.max(pcl[:, 0]) - np.min(pcl[:, 0]), np.max(pcl[:, 1]) - np.min(pcl[:, 1]),
                          np.max(pcl[:, 2]) - np.min(pcl[:, 2])])
    shape_center = [(np.max(pcl[:, 0]) + np.min(pcl[:, 0])) / 2, (np.max(pcl[:, 1]) + np.min(pcl[:, 1])) / 2,
                    (np.max(pcl[:, 2]) + np.min(pcl[:, 2])) / 2]
...
class BaseDataset(Dataset):
    ...
    # def process_pt_source(self, shape_name):
    # pt_source = self.get_data(shape_name)
    ...
    def process_data(self, shape_name):
        ...
        self.pt_source = None
        self.knn_idx = None
        self.pcl_raw_sub = None

        start_time = time.time()
        # pointcloud, normal_gt = self.get_data(shape_name)
        pointcloud = self.get_data(shape_name)
...
#     def get_pcl_raw_sub(self, shape_name):
#         filedir = "./data/PCPNet"
#         pcl = load_data(filedir=filedir, filename=shape_name + '.xyz', dtype=np.float32)[:, :3]
#         ### normalization
#         pcl = normalization(pcl)

#         self.pcl_raw_sub = pcl
    ...
    def __getitem__(self, idx):
        ...
        pidx = np.random.choice(self.pcl_raw.shape[0], self.num_points//2, replace=True)
