...
def parse_arguments():
    ...
    parser.add_argument('--test_input_path', type=str, default='./data/PCPNet')
    parser.add_argument('--mid_result', type=str, default='./midres')
    ...
    # parser.add_argument('--up4X', default=True, type=str, help='conduct 4X twice to get 16X')
    # parser.add_argument('--up8X', default=True, type=str, help='conduct 4X twice to get 16X')
    # parser.add_argument('--up16X', default=True, type=str, help='conduct 4X twice to get 16X')
    parser.add_argument('--double_4X', default=False, type=str, help='conduct 4X twice to get 16X')
...
def interpolate(args):
    # 保存中点插值点云结果
    pcd_dir = os.path.join(args.mid_result, args.data_set)
    if not os.path.exists(pcd_dir):
        os.makedirs(pcd_dir)

    # 检查目录是否存在
    if not os.path.isdir(args.test_input_path):
        print("Error: The specified directory does not exist.")
        return []
    # 获取目录下的所有文件名
    files = os.listdir(args.test_input_path)
    # 去除文件名后缀，只保留文件名
    shape_names = [os.path.splitext(file)[0] for file in files]
    # print(shape_names)
    for shape_name in shape_names:
        if(shape_name == 'list' or shape_name == 'npy' or shape_name == '.ipynb_checkpoints'):
            continue
        print(shape_name)
        test_input_path = os.path.join(args.test_input_path, shape_name + '.xyz')

        pcd = o3d.io.read_point_cloud(test_input_path)
        input_pcd = np.array(pcd.points)
        input_pcd = torch.from_numpy(input_pcd).float().cuda()
        input_pcd = input_pcd.unsqueeze(0)

        input_pcd = rearrange(input_pcd, '1 n c -> 1 c n').contiguous().float().cuda()
        # normalize input
        input_pcd, centroid, furthest_distance = normalize_point_cloud(input_pcd)
        interpolate_pts = midpoint_interpolate(args, input_pcd)
        
        # if args.up4X == True:
        #     interpolate_pts, centroid, furthest_distance = normalize_point_cloud(interpolate_pts)
        #     interpolate_pts = midpoint_interpolate(args, interpolate_pts)
        #     if args.up8X == True:
        #         interpolate_pts, centroid, furthest_distance = normalize_point_cloud(interpolate_pts)
        #         interpolate_pts = midpoint_interpolate(args, interpolate_pts)
        #         if args.up16X == True:
        ...
        # interpolate_pts = centroid + interpolate_pts * furthest_distance
        
        pcl_preprocess = rearrange(interpolate_pts, '1 c n -> 1 n c').contiguous().float().cuda()
        pcl_preprocess = pcl_preprocess.squeeze(0)
         # 将张量移动到主机内存上
        pcl_preprocess_cpu = pcl_preprocess.cpu()
        # save path
        save_path = os.path.join(pcd_dir, shape_name + '.xyz') 
        np.savetxt(save_path, pcl_preprocess_cpu.numpy())
        # np.savetxt(save_path, pcl_preprocess_cpu.numpy(), fmt='%.6f')
...
def train(data_list, log_dir, log_name, ckpt_dir, id=None):
    ...
    for shape_idx, shape_name in enumerate(train_set.cur_sets):
        ...
        # train_set.get_pcl_raw_sub(shape_name)
        ...
        for iter_i in range(1, args.max_iter + 1):
            ...
            # print(f"pcl_raw.shape:{pcl_raw}")
            # print(f"pcl_source.ED.shape:{pcl_source}")
            # print(f"knn_idx.shape:{knn_idx}")
            # print(f"pcl_raw_sub.shape:{pcl_raw_sub}")
            ### Reset gradient and model state
            my_model.train()
            optimizer.zero_grad()

            pcl_source = torch.cat([pcl_source, pcl_raw_sub], dim=-2)
            # print(f"pcl_source.ING.shape:{pcl_source.shape}")
...
def test(data_list):
    ...
    ### Dataset
    test_set = BaseDataset(root=args.mid_result,
                            data_set=args.data_set,
                            data_list=data_list,
                        )
    ...
    for shape_idx, shape_name in enumerate(test_set.cur_sets):
        ...
        # pcl_raw, nor_gt = test_set.get_data(shape_name)         # (N, 3)      #####################这里

        pcl_raw = test_set.get_data(shape_name)
        ...
        if num_point <= max_n:
            ...
            with torch.no_grad():
                grad_norm, pcl_source_result = my_model(pcl_source)
                grad_norm = grad_norm.cpu().detach().numpy()
                pcl_source_result=pcl_source_result.cpu().detach().numpy()
#         else:
...
#         pred_norm /= np.linalg.norm(pred_norm, axis=-1, keepdims=True        
            ...
            pcl_source = torch.from_numpy(pcl_new).float()           # (max_n*k, D)
            grad_norm = np.zeros((pcl_new.shape[0], 3))              # (N, 3)
            pcl_source_result = np.zeros((pcl_new.shape[0], 3))
            with torch.no_grad():
                for i in range(k):
                    grad_norm_s, pcl_source_result = my_model(pcl_source[max_n*i:max_n*(i+1)].to(_device))
                    grad_norm[max_n*i:max_n*(i+1)] = grad_norm_s.cpu().detach().numpy()
                    pcl_source_result[max_n*i:max_n*(i+1)] = pcl_source_result.cpu().detach().numpy()
            grad_norm = grad_norm[:max_n*k-remainder]
            pcl_source_result = pcl_source_result[:max_n*k-remainder]

        ### reorder and normalize the vectors, eliminate zero values
        pred_norm = np.zeros_like(grad_norm)
        pred_pcl_source_result = np.zeros_like(pcl_source_result)
        pred_norm[rand_idxs, :] = grad_norm
        pred_pcl_source_result[rand_idxs, :] = pcl_source_result
        pred_norm[np.linalg.norm(pred_norm, axis=-1) == 0.0] = 1.0
        pred_pcl_source_result[np.linalg.norm(pred_pcl_source_result, axis=-1) == 0.0] = 1.0
        pred_norm /= np.linalg.norm(pred_norm, axis=-1, keepdims=True)
        pred_pcl_source_result /= np.linalg.norm(pred_pcl_source_result, axis=-1, keepdims=True)
        ...
        if args.save_normal_npy or args.save_normal_xyz:
            ...
            if args.save_normal_xyz:
                # pc_nor = np.concatenate([pcl_raw, pred_norm], axis=0)
                # k = 1000; n = 50 # 10
                # pc_nor = pc_nor[n*k:n*k+k, :]
                np.savetxt(path_save + '.xyz', pcl_source_result, fmt='%.6f')
