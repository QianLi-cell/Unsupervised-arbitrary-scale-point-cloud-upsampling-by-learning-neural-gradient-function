...
class Network(nn.Module):
    ...
    def forward(self, pcl_source):
        ...
        return self.grad_sum, pcl_source
    
    def distance_loss(point_cloud, dmin):

        point_cloud = point_cloud.squeeze(0)  # shape: (N, 3)
        N = point_cloud.shape[0]

        # 计算成对距离 (不包括自身距离)
        dist_matrix = torch.cdist(point_cloud, point_cloud, p=2)  # shape: (N, N)

        # 掩盖自身距离 (设置为无穷大)
        dist_matrix.fill_diagonal_(float('inf'))

        # 找到最近邻距离
        nearest_distances, _ = torch.min(dist_matrix, dim=1)  # shape: (N,)

        # 计算损失
        loss = torch.clamp(dmin - nearest_distances, min=0) ** 2
        loss = torch.mean(loss)  # 对所有点取平均
    ...
    def get_loss(self, pcl_raw=None, pcl_source=None, knn_idx=None):
        ...
        """
        # dmin = 
        # loss1 = distance_loss(pcl_raw, dmin)
        ...
        # print(f"pcl_target.shape:{pcl_target}")
        ...
        # print(f"pcl_source_new.shape:{pcl_source_new}")
        ...
        loss_tuple = (loss_d, loss_reg1, loss_reg2)
